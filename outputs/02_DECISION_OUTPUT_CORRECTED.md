...
> `ReadOnly=yes` did not exist as a real mkosi option and has been removed
> below (verified against mkosi's own `config.py`/man page). Immutability
> is applied post-build via `btrfs property set -ts <path> ro true`, not
> by mkosi.
>
> **⚠ STILL PENDING — see `15_PROMPT_NEMOTRON_06_reconcile_arkdep_and_mkosi.md`**
> This document's `KernelCommandLine` still uses `root=LABEL=arch_root` /
> `rootflags=subvol=@`, which assumes the old single-subvolume model. Real
> arkdep addresses each deployment individually
> (`rootflags=subvol=/arkdep/deployments/%target%/rootfs`). Do not build
> against the `subvol=@` line below until file 15's reconciliation is done
> — that part is NOT yet corrected, only the mkosi key names are.
...
# CORRECTED: "ReadOnly=" is not a real mkosi option (confirmed against
# mkosi's own config.py/man page). Immutability is applied AFTER this
# image is deployed, via the deploy tooling running:
#   btrfs property set -ts <deployment_path> ro true
# There is nothing to set here in mkosi.conf for this.
...
# CORRECTED: ReadOnly=yes removed — not a real mkosi key, see note above.
# PENDING FILE 15: the two lines below still assume the old single-@-subvolume
# model. Real arkdep uses root=LABEL=arkane_root (or your chosen label) with
# rootflags=subvol=/arkdep/deployments/%target%/rootfs — do not ship the
# lines below as-is until file 15's reconciliation replaces them.
KernelCommandLine=
    root=LABEL=arch_root
    rootflags=subvol=@
    rw
