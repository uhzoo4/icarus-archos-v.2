...
> **⚠ CORRECTED — see `16_VERIFIED_CORRECTIONS_mkosi_and_arkdep.md`**
> Every `SbsignKey=` / `SbsignCertificate=` reference below has been
> replaced with the real mkosi option names, `SecureBootKey=` and
> `SecureBootCertificate=` (verified against mkosi's own man page/source —
> `SbsignKey`/`SbsignCertificate` never existed). `SecureBoot=yes` is also
> required to enable signing at all, and was missing before.
>
> **⚠ UNVERIFIED** (separate from the fix above — see §9's pre-staging
> block): the `mokutil --root-pw` flag has not been confirmed against real
> `mokutil` source. Check it yourself before relying on it.
...
| `image/mkosi.conf` | Add `SecureBoot=yes`, `SecureBootKey=`, `SecureBootCertificate=` | ☐ Modify |
...
# CORRECTED: real mkosi option names (verified against mkosi's own
# config.py / man page — SbsignKey/SbsignCertificate never existed).
...
> mkosi handles UKI signing natively when `SecureBoot=yes` plus
> `SecureBootKey=`/`SecureBootCertificate=` are set. Your `mkosi.postinst`
> can also call `sbctl sign` explicitly for verification.
...
> **⚠ STILL UNVERIFIED — not covered by the source-code check that fixed the
> mkosi keys above.** `mokutil`/shim aren't in the arkdep or mkosi repos you
> extracted, so this hasn't been checked against real source the way
> `SecureBootKey=` was. `--root-pw` does not match any real `mokutil` flag
> I'm confident exists — real `mokutil --import` prompts interactively for
> a password rather than taking one via a flag. Run `man mokutil` /
> `mokutil --help` yourself before scripting around this; if the flag
> doesn't exist, the whole "pre-staging" idea below needs a different
> mechanism (e.g. an expect script driving the interactive prompt, which
> is a meaningfully different and less clean approach).
